...
- `worker` and `delegate` require a Pi UI confirmation and are denied when `ctx.hasUI` is false. The dialog identifies the role, the workspace, a bounded sanitized task preview, file hints, the commands Agy settings will allow (or that none are), and whether a write rule covers the workspace; it states that the role may edit files and run approved commands, without rendering task, path, context, or file-hint previews; the runner does not use `--sandbox`. The canonical `cwd` is the child process working directory, not a filesystem write sandbox; Agy `write_file(...)` permission rules define the effective write scope.
